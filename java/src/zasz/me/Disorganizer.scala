...
class Disorganizer(var InTags: java.util.Map[String, java.lang.Integer], var Width: Int, var Height: Int)
{
  ...
  var _Increment: (Double) => Double = (X: Double) => X + _Room
  var _Decrement: (Double) => Double = (X: Double) => X - _Room
  ...
  private val _SpiralEndSentinel: Point2D.Double = new Point2D.Double(_MaxEdgeSize + 100, _MaxEdgeSize + 100);
  ...
  private var _Room: Int = 1

  /**
   * Default is 0. Do not go beyond 20.
   * This determines performance : higher = faster.
   * A higher number means a more roomy spiral will place words
   */
  @BeanProperty var SpiralRoom: Int = 0

  private def oddSpiralRoom(): Int =
  {
    2 * SpiralRoom + 1
  }
  ...
  */
  ...
  def GetSpiralNext(PreviousCorner: Point2D.Double): Point2D.Double =
  {
    ...
    else
    {
      ...
      _CurrentEdgeSize = _CurrentEdgeSize + _Room;
      _SleepingEdge = true;
    }
   ...
   *   CloudToCrop : The bitmap of the cloud to crop
   *   returns : The cropped version of the bitmap
   */
